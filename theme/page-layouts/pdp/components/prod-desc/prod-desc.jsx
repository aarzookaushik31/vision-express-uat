  ...
  return (
    <div className={customClass}>
      {isDisplayDataAvailable() && (
        <div
          className={`${styles.descContainerMobile} ${styles.customDescContainer} ${isDescriptionTabs() && styles.isDesktopHidden
            }`}
        >
          {productDescription?.details?.length > 0 && (
            <FyAccordion isOpen={config?.first_accordian_open?.value}>
              {[
                <div className={styles.accordiontitle}>{productDescription?.title}</div>,
                <>
          ...
          {isProductHighlightAvailable() && (
            <FyAccordion>
              {[
                ...
                <ul className={`${styles.pdpDetail} ${styles.items}`}>
          ...
          {getGroupedAttributes().map((attribute, index) =>
            attribute?.details?.length > 0 ? (
              ...
              >
                {[
                  <div className={styles.accordiontitle}>{attribute.title}</div>,
                  <div className={styles.pdpDetail}>
                   <table className={styles.detailTable}>
  <tbody>
    {attribute.details.map((property, val) => (
      <tr key={`${val}-${index}`}>
        <td className={styles.prop}>{property.key}</td>
        <td
